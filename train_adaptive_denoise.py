...
from models.lmfanet import LMFANetAdaptiveDenoise
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for hazy, clear, _ in pbar:
        ...
        loss = criterion(pred, clear, gate)
        loss.backward()
        optimizer.step()

        total_loss += loss.item()
        pbar.set_postfix(loss=f"{loss.item():.4f}", gate=f"{gate.mean().item():.4f}")
...
def validate(model, loader, device):
    ...
    return total_psnr / count, total_ssim / count, total_gate / len(loader)
...
def main():
    parser = argparse.ArgumentParser(description="Train LMFA-Net with adaptive denoising")
    ...
    criterion = CombinedLoss(alpha_ssim=0.02, gate_reg_weight=0.001)
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        f.write("epoch,train_loss,val_psnr,val_ssim,val_gate,time_sec\n")

    for epoch in range(1, args.epochs + 1):
        start_time = time.time()

        train_loss = train_one_epoch(model, train_loader, optimizer, criterion, device)
        val_psnr, val_ssim, val_gate = validate(model, val_loader, device)
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"{epoch},{train_loss:.6f},{val_psnr:.4f},{val_ssim:.4f},{val_gate:.4f},{elapsed:.2f}\n")
