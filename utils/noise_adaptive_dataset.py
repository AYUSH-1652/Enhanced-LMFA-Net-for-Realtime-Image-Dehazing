...
class RESIDEPairedDataset(Dataset):
    ...
    def _add_noise(self, hazy: torch.Tensor) -> torch.Tensor:
        if random.random() >= self.add_noise_prob:
            return hazy

        sigma = random.uniform(self.noise_sigma_min, self.noise_sigma_max)
        noise = torch.randn_like(hazy) * sigma

        if random.random() < self.patch_noise_prob:
            _, h, w = hazy.shape
            ph = random.randint(max(16, h // 4), max(16, h // 2))
            pw = random.randint(max(16, w // 4), max(16, w // 2))
            top = random.randint(0, h - ph)
            left = random.randint(0, w - pw)

            mask = torch.zeros_like(hazy)
            mask[:, top:top + ph, left:left + pw] = 1.0
            hazy = hazy + noise * mask
        else:
            hazy = hazy + noise
